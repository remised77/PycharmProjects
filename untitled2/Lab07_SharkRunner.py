...
def main():
    # Begin with one large try function, as to prevent any red from ever appearing, even when the window is closed unexpectedly.
    #try:
            ...
            while not gui.checkQuitClicked(m1):
                ...
                for i in range(3):
                    ...
                    if sharkObj.returnShark().getX() == allFish[i].returnPoint().getX() and sharkObj.returnShark().getY() == allFish[i].returnPoint().getY():
                        ...
                        # Tell the user if a fish has been eaten.
                ...
                if fishCount == 1:
                    for i in range(3):
                        # A stalemate occurs when a fish is repeatedly able to escape through a wall. This happens when
                        # there is one fish left and the fish is on the same row as the shark. During a chase that results
                        # in a stalemate, there is always a point where the fish is 8 spaces from the shark, on its row, and
                        # facing the shark. This is what we decided to check for to prevent stalemates.
                        ...
                        # or too many turns have occurred (in case of an unforseen stalemate situation).
                        if (yDist == 0 and ((xDist == 8 and allFish[i].returnDirection() == 3) or (xDist == -8 and allFish[i].returnDirection() == 1))) or turns == 40:
                            # It is a stalemate, and the game is over.
                            stalemate = True
                            gameOver = True
                        # Repeated as above.
